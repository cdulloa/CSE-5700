...
Programming Excerise 5: LR(0) sets
...
public:
    char lhs;
    string rhs;
    CanonicalSet(char _lhs, string _rhs) : lhs(_lhs), rhs(_rhs) {}
};

class singleRP
{
private: vector <CanonicalSet*> productions;
public:
    map<int, char> gotos;
    singleRP() {}
    void push(CanonicalSet* p) { productions.push_back(p); }
...
int main()
{
    ...
    while (!input.eof())
    {
        ...
        if (isupper(value))
        {
            ...
            singleRPs[0].push(new CanonicalSet('\'', temp));
        ...
        if (production[value] == "")
        {
            ...
            {
                singleRPs[0].push(new CanonicalSet(value, "." + rhs));
            }
        }
        else
        {
    ...
    for (int i = 0; i < singleRPs[itemid].Size(); i++)
    {
        ...
        if (singleRPs[itemid].gotos.find(lookahead) == singleRPs[itemid].gotos.end())
        {
            if (GlobalGoTo.find(production) == GlobalGoTo.end())
            {
                ...
                singleRPs.back().push(new CanonicalSet(lhs, newRhs));
        ...
        else
        {
            ...
            if (!singleRPs[nextItem].Contains(string(&lhs, 1) + "->" + rhs))
                singleRPs[nextItem].push(new CanonicalSet(lhs, rhs));
            swap(rhs[index], rhs[index + 1]);
            printf("\t%-20s\n", &production[0]);
        }
    }
...
void add_closure(char lookahead, singleRP& item, extendedGrammar& grammar)
{
    ...
    for (int i = 0; i < int(grammar[lookahead].size()); i++)    
    {
        ...
        if (!item.Contains(lhs + "->" + rhs))
            item.push(new CanonicalSet(lookahead, rhs));
    }
}